...
# A lookup table for Big Ten teams with their corresponding team IDs
Big_10_Teams <- tibble(
...
Big10_Stats <- function(team_id, team_name, min_PA = 50) {
  ...
  # Pull raw batting stats for the team
...
# Pull stats for a single team (Washington) as a test
Washington <- Big10_Stats(team_id = 596443, team_name = "Washington")

# Pull combined Big Ten hitters' stats using pmap_dfr()
